...
namespace Microsoft.Extensions.Localization
{
    public class CookieRequestCultureProviderTest
    {
        ...
        [Fact]
        public async Task ParseInvalidCookieShouldLogWarning()
        {
            ...
            {
                var client = server.CreateClient();
                client.DefaultRequestHeaders.Add("Cookie", new CookieHeaderValue("Preferences", "c=??|uic=en-US").ToString());
            ...
            Assert.Equal($"{nameof(CookieRequestCultureProvider)} contains invalid culture '??'.", logMessages[0].State.ToString());
        }
    }
}